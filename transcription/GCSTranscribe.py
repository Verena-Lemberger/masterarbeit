...
first_word_time = words_info[0].start_time
last_word_time = 0
phrase_idx = 0
phrases[phrase_idx] = str(last_speaker_tag) + ": "

# Prepare the phrases including the seconds for the end and the start of each speaker tag
for word_info in words_info:
    # Set last word time for current word
    last_word_time = word_info.end_time

    if (word_info.speaker_tag != last_speaker_tag):
        # Different speaker
        phrases[phrase_idx] += "| start: "+ str(first_word_time.total_seconds()) + " sec., end: " + str(last_word_time.total_seconds()) + " sec."
        # Set to new speaker
        last_speaker_tag = word_info.speaker_tag
        # New phrase
        phrase_idx += 1
        phrases[phrase_idx] = str(last_speaker_tag) + ": "
        first_word_time = word_info.start_time
    
    # Write word to phrase
    phrases[phrase_idx] += str(word_info.word) + " "

# set times for last phrase
phrases[phrase_idx] += "| start: "+ str(first_word_time.total_seconds()) + " sec., end: " + str(last_word_time.total_seconds()) + " sec. END"     
...
# Write phrases to txt file from as "name of the file"transcript.txt 
try:
